...
fish_wb_p = get_weibull_parm(14*365,1/300,upper_bound = c(0.9,5))
fish_s = survival_function(pweibull,list(scale=fish_wb_p$scale,shape=fish_wb_p$shape))

##Use the real fish data as a base
fish_info = read_csv("./data-raw/fish_lite.csv") 

batt_days = c(300,600,1500,2500)
batt_wb_p = lapply(batt_days,function(x){get_weibull_parm(x,0.95,lower_bound = c(1,0),upper_bound = c(10,10))})
...
rec = read_csv("./data-raw/rec_lite.csv")
...
##}
    ...
    while (is.null(curr_indi) || inherits(curr_indi, "try-error")) {
        ...
        curr_indi<- try({
            simulate_individual(i,rel_date,rel_loc,age,fish_s,batt_fn,rec_info,final_rast,map_filtered,cur_b,start_crs=start_crs,mid_crs=mid_crs,end_crs=end_crs,ou_opts=cur_opts,timestep_per_day=0.25)
        }, silent = FALSE) # Suppresses error messages in the console
    }
    indi[[i]] = curr_indi

}
saveRDS(indi,"testIndi.rds")    

no_d = sapply(indi,function(x){if(nrow(x$detections) == 0){TRUE}else{FALSE}})
